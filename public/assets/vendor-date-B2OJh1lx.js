...
function Lt(t, e, n) {
  const r = h(t, n?.in);
  return isNaN(e) ? l(t, NaN) : (r.setDate(r.getDate() + e), r);
}
let N = {};
function O() {
  return N;
...
const R = {
  ...
  V = {
    date: x({ formats: R, defaultWidth: "full" }),
    time: x({ formats: I, defaultWidth: "full" }),
    dateTime: x({ formats: A, defaultWidth: "full" }),
  },
  J = {
...
function p(t) {
  return (e, n = {}) => {
    ...
    const i = o[0],
      ...
      f = Array.isArray(u) ? ot(u, c => c.test(i)) : at(u, c => c.test(i));
    let m;
    ((m = t.valueCallback ? t.valueCallback(f) : f),
...
const g = {
  ...
  S = {
    ...
    k: function (t, e, n) {
      ...
      return (
        r === 0 && (r = 24),
        e === "ko" ? n.ordinalNumber(r, { unit: "hour" }) : s(r, e.length)
      );
    },
    m: function (t, e, n) {
      return e === "mo"
...
const qt = /[yYQqMLwIdDecihHKkms]o|(\w)\1*|''|'(''|[^'])+('|$)|./g,
  Nt = /P+p+|P+|p+|''|'(''|[^'])+('|$)|./g,
  Ht = /^'([^]*?)'?$/,
  Qt = /''/g,
  _t = /[a-zA-Z]/;
function jt(t, e, n) {
  const r = O(),
    ...
    u = h(t, n?.in);
  if (!L(u)) throw new RangeError("Invalid time value");
  let f = e
    .match(Nt)
    .map(d => {
      const c = d[0];
    ...
    .map(d => {
      ...
      const c = d[0];
      if (c === "'") return { isToken: !1, value: Xt(d) };
      if (S[c]) return { isToken: !0, value: d };
      if (c.match(_t))
