...
mysqldump -u root -p sleekinvoices*dev > backup*$(date +%Y%m%d).sql
...
export const authConfig = {  
 ...
 session: {  
 strategy: "jwt",  
 maxAge: 365 _ 24 _ 60 \* 60, // 1 year  
 },  
 pages: {  
 signIn: "/login",  
 error: "/login",  
...
{  
 ...
 {  
 "src": "/(._\\.(css|js|png|jpg|jpeg|gif|svg|ico|woff|woff2|ttf|eot))",  
 "dest": "/dist/public/$1"  
 },  
 {  
 "src": "/(._)",  
...
mysql -h aws.connect.psdb.cloud -u xxx -p sleekinvoices-prod -e "SELECT COUNT(_) FROM users;"  
 mysql -h aws.connect.psdb.cloud -u xxx -p sleekinvoices-prod -e "SELECT COUNT(_) FROM invoices;"
